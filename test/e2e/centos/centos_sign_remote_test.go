...
func TestCentosSigningRemote(t *testing.T) {
	ctx := framework.NewTestCtx(t)
	defer ctx.Cleanup()
	util.AddToFrameworkSchemeForTests(t, ctx)
	centosSigningRemote(t, framework.Global, ctx)
}

func centosSigningRemote(t *testing.T, f *framework.Framework, ctx *framework.TestCtx) {
	...
	err = f.Client.Get(goctx.TODO(), types.NamespacedName{Name: util.SigningRemoteName, Namespace: util.ImageNamespace}, cr)
	assert.NoError(t, err)
	assert.Empty(t, cr.Status.SignedImage)

	// Check for a pod with the correct annotation
	success := util.WaitForPodWithImageCompleted(t, f, ctx, namespace, "cop.redhat.com/owner", "signing-test/signing-app", util.CentosImage, util.RetryInterval, util.Timeout)
